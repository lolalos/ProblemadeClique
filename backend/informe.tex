...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        file/.style={rectangle, draw, font=\scriptsize\ttfamily, align=left, minimum height=1cm},
        container/.style={rectangle, draw, dashed, inner sep=0.4cm},
        arrow/.style={-latex, thick, gray}
    ]
    % Backend files
    \node[file, text width=5.2cm] (clique) {
        \textbf{clique\_simple.py}\\
        Clase: SimpleCliqueSolver\\
        Métodos: \_\_init\_\_(), load\_from\_file(), is\_clique(), greedy\_clique(), etc.\\
        Función main(): menú interactivo.
    };
    \node[file, text width=5.2cm, below=of clique] (informe) {
        \textbf{informe.tex}\\
        Documentación del proyecto en LaTeX.
    };
    \node[container, fit=(clique) (informe), label={[font=\bfseries]above:backend/}] (backend_box) {};

    % Other files
    \node[file, text width=5.2cm, right=3.5cm of clique] (dataset) {
        \textbf{com-dblp.top5000.cmty.txt}\\
        Dataset de comunidades DBLP.
    };
    \node[file, text width=5.2cm, below=of dataset] (readme) {
        \textbf{README.md}\\
        Instrucciones y descripción general.
    };
    \node[file, text width=5.2cm, below=of readme] (test) {
        \textbf{test.bat}\\
        Script para automatizar ejecución en Windows.
    };

    % Connections
    \draw[arrow] (dataset.west) -- (clique.east) node[midway, above, sloped, font=\tiny] {es leído por};
    \draw[arrow] (test.west) -- (clique.east) node[midway, below, sloped, font=\tiny] {ejecuta};
    \draw[arrow] (informe.east) -- (clique.west) node[midway, above, sloped, font=\tiny] {documenta};
    \end{tikzpicture}%
    }
    \caption{Estructura del proyecto y relación entre sus componentes.}
    \label{fig:project_structure_detailed}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{img/esquema.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{img/comunidades.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.25\textwidth]{img/qr_github.png}
